...
const alertMailOptions = {
  ...
  to: process.env.EMAIL_NOTIF_DEST ?? process.env.MAIL_USERNAME
}
...
async function main () {
  ...
  async function check (site: Site): Promise<STATUS> {
    ...
    try {
      ...
      return await checkErrorHandler(site, e as Error, `Failed to make fetch call when making check on ${site.name}, will try again after cooldown. Here is the error:\n`)
    }

    logger.debug(`Web request to "${site.url}" complete, now parsing...`)
    if (res.status !== 200) {
      return await checkErrorHandler(site, `Status code on HTTP request was ${res.status}, not 200`, `Bad status code (${res.status}) on request to "${site.url}", sending error email.`)
    }
    let html: string | null = null
    try {
      html = await res.text()
    } catch (e) {
      return await checkErrorHandler(site, e as Error, `Failed to parse HTML from response after fetching url for ${site.name} (${site.url}). We'll skip the check for now and try again after a cooldown. Here is the error:\n`)
    }
